...
- JS-SDK 目标态统一为 `@wecom/jssdk` + `ww.register`。
- legacy `window.wx` 仅允许作为灰度兼容层，不可作为长期目标实现。
- 允许回退的触发条件：
  - 指定企业微信版本与 `ww.register` 存在兼容性问题。
  - 真机回归中新版注册失败且 legacy 方案验证通过。
  - 发布窗口前关键链路验证未通过，且评估存在上线风险。
- 触发回退时必须同步记录：
  - 回退模块范围
  - 回退起止时间
  - 失败错误码/日志证据
