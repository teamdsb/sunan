...
| ID | 优先级 | 层次 | 工作项 | 验收重点 |
|---|---|---|---|---|
| `M8-W3A-1` | P0 | UI | 通用附件组件 | 选择、上传、进度、预览、下载、二次确认删除/解除关联与失败恢复 |
| `M8-W3A-2` | P0 | API | 全模块附件绑定与解除关联 | 不再要求手工 fileId；解除关联继承记录权限且不删除全局文件 |
| `M8-W3A-2P` | P0 | API/UI/Test | 采购执行清单详情接入 | `DELETE /procurement/orders/{id}/attachments/{fileId}` 解除关联；草稿编辑权限、审计、刷新列表、无权/非草稿拒绝与文件仍可复用均回归 |
...
| `M8-W3B-4` | P0 | Audit | 归档证据替换与删除/解除关联留痕 | 记录操作人、时间、原因、业务对象与文件；不可静默覆盖 |
...
| `M8-W3C-4` | P0 | Test | 文件权限和任务测试 | 无权下载、失败重试、采购附件解除关联权限与文件复用 |
