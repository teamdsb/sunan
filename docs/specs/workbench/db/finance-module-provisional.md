...
## 9. Provisional 升级 gate（M6 冻结）

`finance_board` 从 provisional 升级为正式模块前，至少满足：

- 已补齐原始样表或业务确认稿，并可映射到字段组与打印模板。
- 已确认是否存在独立审批流，并冻结 `templateCode -> wecomTemplateId`（若适用）。
- 已确认导出包最终交付对象、模板格式与字段顺序。
- 已明确与 `finance_attendance` 的职责边界，不出现重复统计口径。
- 已完成 `confirmed/provisional` 字段清单复核并产出升级结论。
