...
- 基线输入：`docs/archive/audits/M6-逐条需求对照表.md`
- 执行原则：
...
- 已完成 `C-2`：
  ...
  - 财务角色可见性与建单能力已纳入集成测试。
- 验收归档：`docs/archive/acceptance/common/acceptance-m6-wavec.md`
...
### Wave D 完成记录（2026-04-22）
- 已完成 `D-1`：
  - 后端对采购列表与报表明细查询增加 3 年窗口校验，超窗返回 `400`。
  - 前端采购列表新增提交日期起止筛选与近 3 年可选范围限制。
  - 采购 OpenAPI 文档同步补充“近 3 年范围”约束说明。
  - 测试覆盖合法窗口与超窗拒绝。
- 已完成 `D-2`：
  - 企业微信上线材料清单补齐归档批次号、目录规范、命名规范，保证可审计追溯。
- 已完成 `D-3`：
  - 可观测文档补齐关键链路日志标准字段与失败注入演练模板。
  - 告警阈值、分级、值班、恢复 SOP 已形成可执行闭环。
- 验收归档：
  - `docs/archive/acceptance/common/acceptance-m6-waved.md`
