...
replaced_by: []
---
# M8-M9 升级总路线图：安全管理数字化闭环（M9 暂停基线）

## 1. 文档定位

本文保留 M8、M9 的总体规划边界。M8 已于 2026-07-12 通过并归档；M9 按用户决定暂停，本文随 M9 计划独立封存，只在用户明确恢复 M9 时重新评审。

本文负责跨里程碑边界、能力依赖和总体退出标准。字段、状态、API、数据库和页面细节应在各 Wave 实施前写入 `docs/specs/safety/` 及相关既有领域规格。

## 1.1 当前调度说明

M7 上线体验与导航修复已于 2026-07-10 验收归档，证据见 `docs/archive/acceptance/common/acceptance-m7-wave6.md`。M8 已于 2026-07-12 验收归档；M9 当前暂停，不自动启动 Wave 1。
...
本文不是当前执行入口。恢复 M9 时，实施顺序仍以 Wave 前置验收和恢复时重新确认的生产基线为准。
...
- 通用底座负责权限、任务、步骤、附件（绑定与受审计解除关联）、签名、定位、消息、打印、导出和审计。
...
4. 统一附件、拍照、签名、定位、打印、导出、下载和受审计解除关联；既有采购执行清单详情必须接入。
