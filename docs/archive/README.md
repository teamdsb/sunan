---
status: current-index
owner: archive
updated: 2026-07-12
replaces: []
replaced_by: []
---
# 文档归档索引

> 状态：历史归档入口。这里保留执行计划、验收记录、审计快照、已取代模板和历史 backlog，便于追溯，不作为新需求的当前真源。

## 目录

| 目录 | 状态 | 内容 |
|---|---|---|
| `execplans/` | 历史归档 | M1-M8 执行计划与 M6 完成快照 |
| `acceptance/common/` | 验收归档 | M6 通用验收、质量门禁、上线包、测试报告 |
| `acceptance/procurement/` | 验收归档 | 采购模块验收 |
| `acceptance/wecom/` | 验收归档 | 企业微信验收与 preflight |
| `acceptance/workbench/` | 验收归档 | 工作平台 M4/M5/M6 验收 |
| `acceptance/safety/` | 验收归档 | M8 Wave 1-7、总验收、迁移、真机待办和上线包 |
| `audits/` | 审计快照 | 特定日期代码或需求审计记录 |
| `backlogs/common/` | 历史归档 | M6-M7 修复清单 |
| `prompts/m7/` | 历史归档 | M7 已完成 Wave 提示词 |
| `backlogs/safety/` | 历史归档 | M8 已完成 Wave backlog |
| `prompts/m8/` | 历史归档 | M8 已完成 Wave 提示词 |
| `paused/m9/` | 暂停基线 | M9 未开始的路线、计划、backlog 和提示词；只在用户明确恢复后启用 |
| `backlogs/workbench/` | 历史归档 | M5 工作平台 backlog |
| `superseded/wecom/` | 已取代 | 被后续企业微信文档替代的模板 |
| `superseded/workbench/` | 已取代 | 被后续工作平台文档替代的记录 |
| `templates/common/` | 模板 | 历史交付模板 |

## 使用规则

- 归档文档只做追溯，不直接驱动新开发。
- 如需恢复某项历史结论，先在当前规格或执行计划中重新立项。
- 修链接和补状态头可以改归档文档；不要改历史验收结论本身。
