...
以 `main` 已实现能力为当前产品边界。下列项目是未来可选增强，不是当前前端请求的缺失接口；前端不展示无后端闭环的功能入口或开发占位。

| 优先级 | 建议接口或契约 | 缺口 | 当前前端处理 |
|---|---|---|---|
| P2 | `GET /office/cases/summary` | 个人办理的待提交、审批中、已办结统计不在原 M2 范围 | 已删除“我的办理”占位卡片及统计暗示 |
| P2 | `GET /office/cases`、`GET /office/cases/:id` | 继续办理和查看审批进度不在原 M2 范围 | 已删除相关功能文案，办事模块只展示真实入口目录 |
...
上述可选增强不阻断现有页面使用。若未来要恢复办理实例、审批进度或个人办理统计，必须先增加正式需求、API/DB 规格与后端实现，再恢复 UI。
...
| 真实 API 冒烟 | 认证、企业资料/制度、证照、监控、提醒、办事、采购、工作台、计划任务与检查 CAPA 共 24 个业务请求返回 200 |
