...
replaced_by: []
---
# M7 Wave 1 提示词：文档、架构与规格基线

```text
你正在苏南船舶管理系统仓库执行 M7 Wave 1。目标是冻结安全管理底座的文档、架构、术语、规格目录和验收门禁，本 Wave 不实现业务代码。
...
完成工作包：
- M7-W1A：复核需求、路线图、执行计划和当前代码差距。
- M7-W1B：创建并冻结 safety 的 domain-boundaries.md、terminology-and-status.md，以及后续 API/DB/state/UI 规格目录。
- M7-W1C：冻结测试矩阵、迁移原则、验收模板和提示词入口。

要求：
1. 先检查 git status，不得覆盖或回滚非本任务改动。
2. 对当前实现的每个关键判断给出文件、接口或测试证据。
3. 明确 M7 不新增第五个一级导航，不接外部监管、AIS、CCTV，不建设小程序。
