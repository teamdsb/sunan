...
replaced_by: []
---
# M8 Wave 1 提示词：M7 基线回归与专业规格冻结

```text
执行 M8 Wave 1。目标是验证 M7 生产底座、冻结 M8 六组专业领域规格，并阻断未解决的 P0 问题。本 Wave 不实现专业业务代码。
...
完成：
1. 回归 M7 权限、流程、证据、主数据、任务、消息、导出和 CAPA。
2. 形成 M7 P0/P1 缺陷清单；存在 P0 时停止 M8 实现。
...
验证：
- 完整执行 M7 自动化回归和 build。
- 校验全部现行 OpenAPI。
- 运行文档索引和 git diff --check。
- 扫描专业规格是否存在重复实体、冲突状态或不明确真源。

最终报告包括 M7 回归结果、主数据质量指标、P0 blocker、冻结的专业规格矩阵、跨域真源矩阵和 Wave 1 验收。只有无 P0 blocker 时才建议进入 Wave 2。
```
