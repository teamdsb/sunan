...
### 阶段 19：新一轮优化版本生产发布
- [x] 恢复上次发布证据，审计 Git 状态、新提交/未提交改动与生产实况
- [x] 核对版本号真源、镜像标签、新 migration 与发布影响面，确定本次目标版本
- [x] 按已验证联动边界更新版本/运维元数据，不改变前端页面版本展示
- [x] 执行 Web/API 测试、构建、lint、OpenAPI、文档和 diff 发布前门禁
- [x] 执行生产只读预检，生成并验证 PostgreSQL/配置/Redis 新备份
- [x] 同步并校验源码，构建新 API/Web/Nginx 镜像，审核/执行必要 migration
- [x] 按 API→Web→Nginx 分阶段切换，完成容器、版本、依赖、路由、鉴权、静态资源和日志验收
- [x] 按 `verification-before-completion` 执行独立新鲜复验，记录发布证据、回滚点和人工验收边界
- **状态：** completed（生产 API/Web/Nginx 均为 `0.0.7`，自动化门禁、备份、Node 20 镜像构建、采购 PDF 字体运行时和公网 smoke 全部通过；企业微信三端真机仍由用户现场执行）

### 阶段 20：最新版真实功能手册与截图替换
- [x] 盘点 `docs/handbook/image/` 中的截图、尺寸、图号与当前手册占位位
- [x] 对照当前源码/规格逐节审计手册，删除不存在的功能、入口和操作步骤
- [x] 将真实截图嵌入 Markdown，并为缺失截图保留 4:3 占位图
- [x] 从 Markdown 重新生成 DOCX，检查图片比例、图注、版式和章节内容
- [x] 执行 Markdown/DOCX/链接/术语/渲染全量验证并更新文档索引与进度
- **状态：** completed（0.0.7 手册已按源码核对；40 张真实截图已嵌入，34 个正式图位保留 4:3 占位；DOCX 校验、A4 渲染和 stale/M9/Wave 扫描通过）

### 阶段 21：移除实际系统不存在的安全管理操作
- [x] 以用户确认的实际可见系统为最终口径，盘点手册内全部安全管理残留
- [x] 删除安全主数据、计划、任务、检查、问题、CAPA 及相关角色、路由、FAQ、合规和维护说明
- [x] 删除不存在的工作平台安全/检查整改模块并连续重排章节、图号和引用
- [x] 重新生成 DOCX，更新目录页码并验证 Markdown/Word 内容一致
...
| 手册以用户实际可见系统为唯一口径 | 源码仍存在的安全路由不代表用户系统可操作；手册不保留隐藏入口、历史安全流程或“功能存在但当前不显示”的提示 |
...
### 阶段 29：0.0.7 工作台布局修改再次生产发布
- [x] 审计工作树、版本真源、迁移文件和生产当前状态
- [x] 保持全局版本 `0.0.7`，不修改前端页面版本显示
- [x] 执行 Web/API 测试、构建、lint、OpenAPI、文档索引和 diff 门禁
- [x] 完成生产备份、源码原子同步和旧镜像回滚标签
- [x] 构建并切换新 Web 镜像，保持 API/Nginx 已验证镜像不变
- [x] 新鲜验证六个服务、数据库、关键接口、公网路由、CSS/JS、日志、TLS、定时器和备份
- **状态：** completed（本轮运行时变更仅为工作台响应式布局 CSS；生产 Web 新摘要 `sha256:6a3876d31ed7c795e62eb1ed3436916083644656943c3552d10b1272eab37fb5`，API/Nginx 摘要保持不变；版本仍为 `0.0.7`，25 条 migration；备份批次 `20260831232537`、源码批次与回滚标签 `20260831232655`；8 条 SPA 路由、最新 `index-BraWNGPA.css`、live/ready/401、TLS、定时器和近期日志复验通过）
