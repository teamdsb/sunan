...
export function ProcurementReportPage() {
  ...
  >(undefined);
  const [detailsDepartmentCode, setDetailsDepartmentCode] =
    useState<ProcurementDepartmentCode>('business_dept');
  const [dimensionDepartmentCode, setDimensionDepartmentCode] = useState<
  ...
  const [dimensionDateForm] = Form.useForm<{
    startDate: string;
    endDate: string;
  }>();
  ...
  const [dimensionStartDate, setDimensionStartDate] =
  ...
  const { data: monthlyResponse, isLoading: isMonthlyLoading } =
    useGetProcurementMonthlyReportQuery({ year, month, departmentCode });
  const { data: yearlyResponse, isLoading: isYearlyLoading } =
    useGetProcurementYearlyReportQuery({ year, departmentCode });
  ...
  const { data: dimensionResponse, isLoading: isDimensionLoading } =
    useGetProcurementDimensionDetailsQuery({
      ...
      dimensionType:
        dimensionDepartmentCode === 'shipping_dept'
          ? 'vessel'
          : 'logistics_category',
  ...
  const { data: requestResponse, isLoading: isRequestLoading } =
    useGetProcurementReportRequestsQuery({ page: requestPage, pageSize: 10 });

  const [createReportRequest, { isLoading: isCreatingRequest }] =
    useCreateProcurementReportRequestMutation();
  ...
  const requestColumns: ColumnsType<ProcurementReportRequest> = useMemo(
    () => [
      { title: '单号', dataIndex: 'reportNo', key: 'reportNo', width: 180 },
      {
        ...
        width: 120,
        render: (value: string) =>
          labelFrom(reportTypeLabelMap, value, '其他报表'),
      },
      {
        title: '周期',
        key: 'period',
      ...
      },
      {
        ...
        width: 140,
        render: (value: string) => (
          <Tag color={reportStatusColor[value] ?? 'default'}>
            {labelFrom(reportStatusLabelMap, value, '未知状态')}
          </Tag>
        ),
      },
      {
  ...
  const handleCreateMonthlyRequest = async () => {
    const created = await createReportRequest({
      reportType: 'monthly',
      periodYear: year,
      periodMonth: month,
      departmentCode,
    ...
    <>
      ...
      <section className="page-hero">
        ...
        <Typography.Paragraph type="secondary">
          支持月报、年报、部门明细与部门细分明细查询，并可发起报表审批单。
        ...
        <div className="sunan-query-grid">
          ...
          <Button
            ...
            onClick={() => void handleCreateMonthlyRequest()}
          >
            生成月报审批单
          </Button>
          <Button
            loading={isCreatingRequest}
            onClick={() => void handleCreateYearlyRequest()}
          >
            生成年报审批单
          </Button>
          <Button onClick={() => navigate('/procurement/report-approvals')}>
            进入报表审批页
          </Button>
        </div>
      </section>
      ...
      <section className="page-card-grid">
        ...
        >
          <ResponsiveTable
            ...
            dataSource={monthlyResponse?.data.items ?? []}
            pagination={false}
          />
        </Card>
      </section>
      ...
      <section className="page-card-grid">
        ...
        >
          <ResponsiveTable
            ...
            columns={monthlyColumns}
            dataSource={yearlyResponse?.data.items ?? []}
            pagination={false}
          />
        </Card>
      </section>
      ...
      <section className="page-card-grid">
        <Card
          ...
          title="部门采购明细"
        >
          <div className="sunan-query-grid" style={{ marginBottom: 16 }}>
            <Select
              value={detailsDepartmentCode}
              options={departmentOptions}
              onChange={(value) => setDetailsDepartmentCode(value)}
            />
            <Form
              form={dateRangeForm}
              layout="vertical"
          ...
          <ResponsiveTable
            ...
            columns={[
              ...
              },
              {
                ...
                key: 'status',
                width: 140,
                render: (value: string) => reportStatusLabelMap[value] ?? value,
              },
            ]}
          />
        </Card>
      ...
      <section className="page-card-grid">
        ...
        >
          <ResponsiveTable
            ...
            dataSource={requestResponse?.data ?? []}
            pagination={false}
          />
