...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      autoSave: false,
      synchronize: true,
      entities: [ShipMonitorEntity, VesselEntity],
    }),
    ShipMonitorModule,
  ],
})
...
describe('ShipMonitorController integration', () => {
  ...
  it('supports manager CRUD and employee readonly(active only)', async () => {
    const create1 = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/api/v1/ship-monitors')
      .set('Authorization', 'Bearer token')
      .send({ vesselId, monitorName: '主监控', endpointUrl: 'https://monitor1.example.com' });
    const id1 = (create1.body as { data: { id: string } }).data.id;
    expect(create1.status).toBe(201);

    const create2 = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/api/v1/ship-monitors')
      .set('Authorization', 'Bearer token')
      .send({ vesselId, monitorName: '备用监控', endpointUrl: 'https://monitor2.example.com' });
    ...
    currentUser = { ...currentUser, userId: 'employee-1', roles: ['all_authenticated'], position: '员工' };
    const employeeList = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .get('/api/v1/ship-monitors?activeOnly=false')
      .set('Authorization', 'Bearer token');
    expect(employeeList.status).toBe(200);
    expect((employeeList.body as { data: Array<{ id: string }> }).data).toHaveLength(1);
    expect((employeeList.body as { data: Array<{ id: string }> }).data[0]?.id).toBe(id1);

    const employeeCreate = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .post('/api/v1/ship-monitors')
      .set('Authorization', 'Bearer token')
      .send({ vesselId, monitorName: '员工创建', endpointUrl: 'https://monitor3.example.com' });
    expect(employeeCreate.status).toBe(403);

    currentUser = { ...currentUser, userId: 'manager-1', roles: ['all_authenticated', 'shipping'], position: '经理' };
