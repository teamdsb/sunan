...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      autoSave: false,
      synchronize: true,
      entities: [EnterprisePolicyEntity, EnterprisePolicyFileEntity, FileEntity, WecomUserEntity],
    }),
...
describe('EnterprisePolicyController integration', () => {
  ...
  beforeAll(async () => {
    ...
    const file = await fileRepo.save(fileRepo.create({ ossKey: 'enterprise-policies/2026/03/a.pdf', fileName: 'a.pdf', mimeType: 'application/pdf', fileSize: 123, category: 'enterprise-policies', uploadedBy: 'manager-1' }));
    fileId = file.id;
  });
  ...
  it('supports create/list/detail/versions/publish/bind and blocks employee write', async () => {
    ...
    const versions = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .get(`/api/v1/enterprise-policies/${id}/versions`)
      .set('Authorization', 'Bearer token');
    expect(versions.status).toBe(200);

    const publish = await request(app.getHttpServer() as Parameters<typeof request>[0])
