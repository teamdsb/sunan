...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      autoSave: false,
      synchronize: true,
      entities: [EnterpriseProfileEntity, EnterpriseProfileFileEntity, FileEntity, WecomUserEntity],
    }),
...
describe('EnterpriseProfileController integration', () => {
  ...
  beforeAll(async () => {
    ...
    await wecomRepo.save([
      wecomRepo.create({ userId: 'manager-1', corpId: 'ww-test', name: 'Manager1', avatarUrl: null, departmentCodes: ['business_dept'], departmentNames: ['业务部'], position: '经理', isSystemAdmin: false, rawProfile: {} }),
      wecomRepo.create({ userId: 'manager-2', corpId: 'ww-test', name: 'Manager2', avatarUrl: null, departmentCodes: ['finance_dept'], departmentNames: ['财务部'], position: '经理', isSystemAdmin: false, rawProfile: {} }),
      wecomRepo.create({ userId: 'employee-1', corpId: 'ww-test', name: 'Employee', avatarUrl: null, departmentCodes: ['business_dept'], departmentNames: ['业务部'], position: '员工', isSystemAdmin: false, rawProfile: {} }),
    ]);

    const fileRepo = dataSource.getRepository(FileEntity);
    const file = await fileRepo.save(fileRepo.create({ ossKey: 'enterprise-profiles/2026/03/a.pdf', fileName: 'a.pdf', mimeType: 'application/pdf', fileSize: 123, category: 'enterprise-profiles', uploadedBy: 'manager-1' }));
    fileId = file.id;
  });
  ...
  it('blocks normal employee from create and allows manager CRUD with file bind/unbind', async () => {
    currentUser = { ...currentUser, userId: 'employee-1', roles: ['all_authenticated'] };
    ...
    currentUser = { ...currentUser, userId: 'manager-1', roles: ['all_authenticated', 'business'] };
    ...
    expect((bind.body as { data: { files: Array<{ id: string }> } }).data.files[0]?.id).toBe(fileId);

    const unbind = await request(app.getHttpServer() as Parameters<typeof request>[0])
      .delete(`/api/v1/enterprise-profiles/${id}/files/${fileId}`)
      .set('Authorization', 'Bearer token');
    expect(unbind.status).toBe(204);

    const remove = await request(app.getHttpServer() as Parameters<typeof request>[0])
  ...
  it('enforces department scope for managers', async () => {
    ...
    currentUser = { ...currentUser, userId: 'manager-2', roles: ['all_authenticated', 'finance'] };
    const update = await request(app.getHttpServer() as Parameters<typeof request>[0])
