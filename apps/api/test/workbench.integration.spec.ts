...
const buildCallbackSignature = (payload: {
  ...
  const signature = sha1(
    ['test-callback-token', timestamp, nonce, payloadDigest].sort().join(''),
  );
...
describe('WorkbenchController integration', () => {
  ...
  it('persists records, steps, attachments, print snapshots and approval sync', async () => {
    const createFlowResponse = await request(
    ...
    const flowRecordId = (createFlowResponse.body as { data: { id: string } })
      .data.id;

    const listFlowResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    expect(
      (listFlowResponse.body as { data: Array<{ id: string }> }).data,
    ).toEqual(
    ...
    expect(
      (startResponse.body as { data: { status: string } }).data.status,
    ).toBe('in_progress');

    const completeStepResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post(`/api/v1/workbench/records/${flowRecordId}/actions`)
      .set('Authorization', 'Bearer token')
      .send({
        ...
        payload: { stepCode: 'pre_shift_meeting' },
      });
    ...
    const uploadAttachmentResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      ...
      .send({
        category: 'evidence',
        fileId: file.id,
        stepCode: 'pre_shift_meeting',
      });

    expect(uploadAttachmentResponse.status).toBe(201);
    expect(
      (
        uploadAttachmentResponse.body as {
          data: {
            fileId: string;
            fileName: string;
            mimeType: string;
    ...
    const attachmentDownloadSpy = jest
      .spyOn(ossService, 'createDownloadSignature')
      .mockResolvedValue({
        downloadUrl: 'https://oss.example.test/workbench-attachment',
        expiresAt: '2026-08-04T12:00:00.000Z',
      });
    const attachmentDownloadResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get(
        `/api/v1/workbench/records/${flowRecordId}/attachments/${file.id}/download-url`,
      )
      .set('Authorization', 'Bearer token');
    expect(attachmentDownloadResponse.status).toBe(200);
    expect(attachmentDownloadResponse.body).toEqual({
      data: {
        downloadUrl: 'https://oss.example.test/workbench-attachment',
        expiresAt: '2026-08-04T12:00:00.000Z',
      },
    });
    attachmentDownloadSpy.mockRestore();

    const unboundAttachmentDownloadResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get(
        `/api/v1/workbench/records/${flowRecordId}/attachments/00000000-0000-0000-0000-000000000001/download-url`,
      )
      .set('Authorization', 'Bearer token');
    expect(unboundAttachmentDownloadResponse.status).toBe(404);
    ...
    expect(
      (
        printResponse.body as {
          data: { recordId: string; renderedFormat: string };
        }
      ).data,
    ).toMatchObject({
    ...
    const createApprovalRecordResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/workbench/records')
      .set('Authorization', 'Bearer token')
    ...
    const approvalRecordId = (
      createApprovalRecordResponse.body as { data: { id: string } }
    ).data.id;

    const launchApprovalResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    expect(
      (
        callbackResponse.body as {
          data: { ignored: boolean; mirrorStatus: string };
        }
      ).data.ignored,
    ).toBe(false);
    expect(
      (callbackResponse.body as { data: { mirrorStatus: string } }).data
        .mirrorStatus,
    ).toBe('approval_passed');
    ...
    const duplicateCallbackResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/wecom/approval/callback')
    ...
    expect(
      (duplicateCallbackResponse.body as { data: { ignored: boolean } }).data
        .ignored,
    ).toBe(true);

    const invalidSignatureCallback = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/wecom/approval/callback')
    ...
    const instanceListForbiddenResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const instanceListResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get('/api/v1/wecom/approval/instances')
      .set('Authorization', 'Bearer token')
    ...
    expect(
      (
        instanceListResponse.body as {
          data: Array<{ processInstanceId: string }>;
        }
      ).data,
    ).toEqual(
    ...
    expect(
      (retryResponse.body as { data: { accepted: boolean } }).data.accepted,
    ).toBe(true);
    ...
    const reconcileResponse = await request(
      ...
      .send({
        ...
        reason: 'monthly reconcile',
      });

    expect(reconcileResponse.status).toBe(202);
    expect(
      (reconcileResponse.body as { data: { acceptedCount: number } }).data
        .acceptedCount,
    ).toBe(1);

    const instanceResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const storedRecord = await recordRepository.findOneByOrFail({
      id: flowRecordId,
    });
    expect(storedRecord.recordNo).toEqual(
      expect.stringMatching(/^WB\d{8}[A-Z0-9]{8}$/),
    );

    const approvalSyncRepository = dataSource.getRepository(
      WecomApprovalInstanceSyncEntity,
    );
    const approvalSync = await approvalSyncRepository.findOneByOrFail({
      processInstanceId,
    });
  ...
  it('exposes wave-a and wave-b schema fields and enforces required payload fields', async () => {
    ...
    const shipSignSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const shipSignFields = (
      (
        shipSignSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
    ...
    const vesselDynamicSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const vesselDynamicFields = (
      (
        vesselDynamicSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
      ).data.sections[0]?.fields ?? []
    ).map((field) => field.key);
    ...
    const garbageSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const garbageFields = (
      (
        garbageSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
      ).data.sections[0]?.fields ?? []
    ).map((field) => field.key);
    expect(garbageFields).toEqual(
      expect.arrayContaining(['nationality', 'voyageNo', 'fee']),
    );

    const oilyWaterSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get(
        '/api/v1/workbench/modules/business_ship_oily_water_operation/schema',
      )
      .set('Authorization', 'Bearer token');
    expect(oilyWaterSchemaResponse.status).toBe(200);
    const oilyWaterFields = (
      (
        oilyWaterSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
      ).data.sections[0]?.fields ?? []
    ).map((field) => field.key);
    expect(oilyWaterFields).toEqual(
      expect.arrayContaining(['nationality', 'documentNo', 'fee']),
    );

    const domesticSewageSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get(
        '/api/v1/workbench/modules/business_domestic_sewage_operation/schema',
      )
      .set('Authorization', 'Bearer token');
    expect(domesticSewageSchemaResponse.status).toBe(200);
    const domesticSewageFields = (
      (
        domesticSewageSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
      ).data.sections[0]?.fields ?? []
    ).map((field) => field.key);
    expect(domesticSewageFields).toEqual(
      expect.arrayContaining(['nationality', 'documentNo', 'voyageNo', 'fee']),
    );

    const trainingSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const trainingFields = (
      (
        trainingSchemaResponse.body as {
          data: { sections: Array<{ fields: Array<{ key: string }> }> };
        }
      ).data.sections[0]?.fields ?? []
    ).map((field) => field.key);
    ...
    const trainingRecordId = (
      createTrainingResponse.body as { data: { id: string } }
    ).data.id;

    const updateTrainingPayloadResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    const missingRequiredFieldResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/workbench/records')
      .set('Authorization', 'Bearer token')
    ...
    expect(
      (missingRequiredFieldResponse.body as { message: string }).message,
    ).toContain('payload.watchVessel');

    const validCreateResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/workbench/records')
      .set('Authorization', 'Bearer token')
    ...
    expect(
      (validCreateResponse.body as { data: { moduleCode: string } }).data
        .moduleCode,
    ).toBe('business_ship_sign');
    ...
    const printDefaultResponse = await request(
      ...
      .get(
        `/api/v1/workbench/records/${(validCreateResponse.body as { data: { id: string } }).data.id}/print`,
      )
      .set('Authorization', 'Bearer token');
    expect(printDefaultResponse.status).toBe(200);
    expect(
      (printDefaultResponse.body as { data: { paperSize: string } }).data
        .paperSize,
    ).toBe('A4');

    const printA3Response = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get(
        `/api/v1/workbench/records/${(validCreateResponse.body as { data: { id: string } }).data.id}/print`,
    ...
    expect(
      (printA3Response.body as { data: { paperSize: string } }).data.paperSize,
    ).toBe('A3');
  });
  ...
  it('aligns workbench module approval matrix and fuel bunkering service asset approval flow', async () => {
    ...
    const modulesByCode = new Map(
      ...
      ).data.map((moduleItem) => [moduleItem.moduleCode, moduleItem]),
    );
    ...
    expect(modulesByCode.get('shipping_attendance')?.requiresApproval).toBe(
      true,
    );
    ...
    const fuelSchemaResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    expect(
      (fuelSchemaResponse.body as { data: { templateType: string } }).data
        .templateType,
    ).toBe('service_asset');
    const fuelFields = (
      (
        fuelSchemaResponse.body as {
          data: {
            sections: Array<{
    ...
    expect(
      (
        createFuelResponse.body as {
          data: { moduleCode: string; status: string };
        }
      ).data,
    ).toMatchObject({
      moduleCode: 'shipping_fuel_bunkering_approval',
    ...
    const fuelRecordId = (createFuelResponse.body as { data: { id: string } })
      .data.id;

    const approvalRecordsResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get('/api/v1/workbench/records')
      .set('Authorization', 'Bearer token')
    ...
    const approvalRecords = (
      approvalRecordsResponse.body as {
        data: Array<{ id: string; moduleCode: string }>;
      }
    ).data;
    expect(approvalRecords).toEqual(
      expect.arrayContaining([expect.objectContaining({ id: fuelRecordId })]),
    );
    expect(
      approvalRecords.every(
        (record) =>
          modulesByCode.get(record.moduleCode)?.requiresApproval === true,
      ),
    ).toBe(true);

    const launchFuelApprovalResponse = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/wecom/approval/launch')
      .set('Authorization', 'Bearer token')
    ...
    expect(
      (
        launchFuelApprovalResponse.body as {
          data: { processInstanceId: string; launchStatus: string };
        }
      ).data,
    ).toMatchObject({
      launchStatus: 'prepared',
  ...
  it('enforces module visibility by role (permission matrix baseline)', async () => {
    ...
    const financeModules = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .get('/api/v1/workbench/modules')
      .set('Authorization', 'Bearer token');
    ...
    const moduleCodes = (
      financeModules.body as { data: Array<{ moduleCode: string }> }
    ).data.map((item) => item.moduleCode);
    ...
    const financeBoardCreate = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post('/api/v1/workbench/records')
      .set('Authorization', 'Bearer token')
    ...
    expect(
      (financeBoardCreate.body as { data: { moduleCode: string } }).data
        .moduleCode,
    ).toBe('finance_business_board');
    ...
    const attendanceExport = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    expect(
      (attendanceExport.body as { data: { status: string } }).data.status,
    ).toBe('queued');

    const attendanceReconcile = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
    ...
    expect(
      (attendanceReconcile.body as { data: { status: string } }).data.status,
    ).toBe('queued');
  ...
  it('rejects a visible non-executor step action and permits the assigned executor', async () => {
    ...
    const record = await recordRepository.save(
      recordRepository.create({
        ...
        departmentCode: 'shipping',
        vesselId: 'sunan-999',
        ownerUserId: 'shipping-owner-2',
        applicantUserId: 'shipping-owner-2',
        assigneeUserId: 'shipping-owner-2',
        reviewerUserId: null,
        occurredAt: new Date(),
        submittedAt: null,
        closedAt: null,
    ...
    const assign = await request(
      ...
      .post(`/api/v1/workbench/records/${record.id}/participants`)
      .set('Authorization', 'Bearer token')
      .send({
        userId: 'executor-1',
        role: 'executor',
    ...
    currentUser = {
      ...currentUser,
      userId: 'crew-visible-not-executor',
      roles: ['all_authenticated', 'crew'],
      departments: ['vessel:sunan-999'],
      isAdmin: false,
    };
    const forbidden = await request(
      ...
      .post(`/api/v1/workbench/records/${record.id}/actions`)
      .set('Authorization', 'Bearer token')
      .send({ actionType: 'complete_step', payload: { stepCode: 'inspect' } });
  ...
  it('prevents an outside crew member from attaching or printing another vessel record', async () => {
    ...
    const record = await recordRepository.save(
      recordRepository.create({
        ...
        applicantUserId: 'owner-evidence',
        assigneeUserId: 'owner-evidence',
        reviewerUserId: null,
        occurredAt: new Date(),
        submittedAt: null,
        closedAt: null,
    ...
    const print = await request(
      ...
      .get(`/api/v1/workbench/records/${record.id}/print`)
      .set('Authorization', 'Bearer token');
    const attachment = await request(
      app.getHttpServer() as Parameters<typeof request>[0],
    )
      .post(`/api/v1/workbench/records/${record.id}/attachments`)
      .set('Authorization', 'Bearer token')
      .send({
        category: 'evidence',
  ...
  it('audits an administrator sensitive record view', async () => {
    const recordRepository = dataSource.getRepository(WorkbenchRecordEntity);
    const actionLogRepository = dataSource.getRepository(
    ...
    const record = await recordRepository.save(
      recordRepository.create({
        ...
        applicantUserId: 'owner-audit',
        assigneeUserId: 'owner-audit',
        reviewerUserId: null,
        occurredAt: new Date(),
        submittedAt: null,
        closedAt: null,
    ...
    expect(
      await actionLogRepository.exist({
        where: {
          businessRecordId: record.id,
          actionType: 'sensitive_view',
          operatorUserId: 'system-admin-audit',
        },
      }),
    ).toBe(true);
  });
